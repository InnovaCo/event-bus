...
define(function(require, exports, module) {
	...
	return {
		...
		on: function(channel, events, callback, context) {
			var event,
				subscription,
				proxyCallback = function(data, envelope) {
					var modifier;

					if (arguments.length === 3) {
						modifier = arguments[0];
						data = arguments[1];
						envelope = arguments[2];
					}

					if (typeof callback === 'function') {
						if (modifier === modifiers.ALL) {
							return callback.apply(this, [envelope.topic].concat(data.__args));
						}
			...
			while (event = events.shift()) {
				subscription = postal.subscribe({
					channel: channel,
					topic: event === 'all' ? '*' : event,
					callback: event === 'all' ? proxyCallback.bind(null, modifiers.ALL) : proxyCallback
				});
